...
public class 카카오2022인턴십_등산코스정하기2 {
        ...
        public int[] solution(int n, int[][] paths, int[] gates, int[] summits) {
            ...
            for (int i = 0; i < gates.length; i++) {
                for (int j = 0; j < summits.length; j++) {
                    ...
                    int max = dijkstra(n, gates[i], tempSummit,links);
...
//                    }
                    if (visited[linkedNode]) continue;
                    int cost = getCost(links, cur, linkedNode);
                    if (cost > max) max = cost;
                    if (costs[linkedNode] > costs[cur] + cost){
                        costs[linkedNode] = costs[cur] + cost;
                    }
